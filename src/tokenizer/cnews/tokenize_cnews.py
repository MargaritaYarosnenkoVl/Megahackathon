...
def write_keywords(df: pd.DataFrame, names: list):
    for name in names:
        with open(f"key_words/key_words_{name}.txt", "a") as f:
            # print(name, ":")
...
if __name__ == "__main__":
    with open("../../parse_news/cnews.json", "r") as f:
        data = json.load(f)
        titles: list[str] = [news.get('title').strip()[9:15] for news in data]
        full_texts: list[str] = [news.get('full_text').strip() for news in data]
        lemmatized_tokens = (get_clear_tokens(text) for text in full_texts)
        df = vectorize(lemmatized_tokens, list(range(len(titles))))
        write_keywords(df, list(range(len(titles))))
