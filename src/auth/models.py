...
class User(SQLAlchemyBaseUserTable[int], Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    full_name: Mapped[str | None] = mapped_column(String, default="Иванов Иван Иванович")
    phone_number: Mapped[str | None] = mapped_column(String, default="9871234567")
    username: Mapped[str] = mapped_column(String, nullable=False, unique=True)
    hashed_password: Mapped[str] = mapped_column(String(length=1024), nullable=False)
    registred_at: Mapped[TIMESTAMP | None] = mapped_column(TIMESTAMP, default=datetime.utcnow)
    role_name: Mapped[str | None] = mapped_column(String, ForeignKey(Role.name))
